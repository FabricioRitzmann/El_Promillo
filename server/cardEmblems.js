...
export function localCardEmblemUrl(cardInstance = {}, appBaseUrl = '') {
  const emblem = cardEmblemMeta(cardInstance);

  if (!appBaseUrl) {
    return emblem.publicPath;
  }

  return new URL(emblem.publicPath, appBaseUrl).toString();
}

export function supabaseCardEmblemUrl(cardInstance = {}, supabaseUrl = '') {
  ...
  return `${baseUrl}/storage/v1/object/public/wallet-emblems/${cardEmblemMeta(cardInstance).storagePath}`;
}
