...
async function claimCard(walletPlatform = 'apple') {
  ...
  try {
    result = await claimCardViaEdge(walletPlatform, walletObjectId, customerIdentityToken, crmRegistration);
  } catch (error) {
    if (!error.fallbackToLocal) {
      throw error;
    }

    result = await claimCardViaLocalApi(walletPlatform, walletObjectId, customerIdentityToken, crmRegistration);
  }
...
async function claimCardViaEdge(walletPlatform, walletObjectId, customerIdentityToken, crmRegistration = null) {
  const supabaseUrl = publicConfig?.supabase?.url?.replace(/\/$/, '');
  const anonKey = publicConfig?.supabase?.anonKey;
...
async function claimCardViaLocalApi(walletPlatform, walletObjectId, customerIdentityToken, crmRegistration = null) {
  const payload = {
    templateId: template.id,
    claimToken: currentClaimToken || undefined,
    walletPlatform,
