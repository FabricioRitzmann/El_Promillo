...
async function claimCardViaEdge(walletPlatform, walletObjectId, customerIdentityToken, crmRegistration = {}) {
  const supabaseUrl = publicConfig?.supabase?.url?.replace(/\/$/, '');
  const anonKey = publicConfig?.supabase?.anonKey;
...
async function claimCardViaLocalApi(walletPlatform, walletObjectId, customerIdentityToken, crmRegistration = {}) {
