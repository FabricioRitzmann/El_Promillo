...
- Die öffentliche Claim-Seite escaped ausgegebene Kartenwerte, Währung und Auflade-Attribute mit `escapeHtml`; Google-Save-Links werden vor dem Rendern mit `safeGoogleWalletSaveUrl(...)` auf `https://pay.google.com/gp/v/save/...` validiert. Dadurch kann ein fehlerhafter oder manipulierter Claim-/Google-Response nicht direkt als unsicheres `href` oder HTML in die mobile Seite gelangen.
