...
- Vor echten Wallet-Aktionen `node scripts/wallet-remote-schema-check.js --strict` ausführen. `Wallet Remote Supabase Schema Check` prüft, ob Tabellen und Spalten aus `supabase/schema.sql` im Supabase REST-Schema erreichbar sind und ob die öffentlichen Storage Buckets `wallet-assets`, `business-logos` und `wallet-emblems` existieren; Secrets und Tokens werden nicht ausgegeben.
