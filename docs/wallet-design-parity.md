...
|---|---|---|---|---|---|---|---|---|
| Logo / Business-Branding | Logo oben links, Businessname daneben | `logo.png`, `icon.png`, optional `thumbnail/strip` oder `combined_emblem` | `logo`, optional `heroImage`/`imageModulesData` inklusive `combined_emblem` | `logoImage`, `appLinkLogo`, `mainImg` je nach Template | Teilweise | Bildfelder sind pro Plattform fest platziert | Business-Logo bevorzugen, sonst Emblem/neutrales Asset; bei begrenzten Slots als `combined_emblem` PNG | Implementiert |
