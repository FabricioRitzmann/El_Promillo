...
| Barcode / QR-Code | Claim-/Karten-Code, optionales Template-Format `qr`, `aztec`, `pdf417` oder `code128`; Editor-Plattformvorschau zeigt das normalisierte Format | `PKBarcodeFormatQR`, `PKBarcodeFormatAztec`, `PKBarcodeFormatPDF417`, `PKBarcodeFormatCode128` | `QR_CODE`, `AZTEC`, `PDF_417`, `CODE_128` | Samsung Barcode-Attribute `serialType`/`ptFormat`/`ptSubFormat`, Template-abhaengig | Teilweise | Plattformformatnamen unterscheiden sich; Samsung-Templates koennen einzelne Formate begrenzen | QR bleibt Default; Editor-/Template-Format wird serverseitig und in der Editor-Vorschau normalisiert; Samsung bekommt bei Nicht-QR einen UI-Hinweis | Implementiert |
