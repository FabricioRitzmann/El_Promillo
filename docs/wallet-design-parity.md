...
| Update Queue fuer Design- und Emblem-Aenderungen | `supabase/schema.sql`, `scanner-actions`, `wallet_update_queue`, `samsung_wallet_events` | Implementiert fuer Apple/Google Queue-Jobs inklusive `emblem_changed`, automatische PNG-Fallbacks vor Updates und Samsung Update-Vorbereitung |
...
|---|---|---|---|---|---|---|---|---|
| Logo / Business-Branding | Logo oben links, Businessname daneben | `logo.png`, `icon.png`, optional `thumbnail/strip` oder `combined_emblem`; Pass-Dateien bekommen nur echte PNG-Bytes | `logo`, optional `heroImage`/`imageModulesData` inklusive `combined_emblem` | `logoImage`, `appLinkLogo`, `mainImg` je nach Template | Teilweise | Bildfelder sind pro Plattform fest platziert; JPEG/WebP duerfen bei Apple nicht unter PNG-Dateinamen verpackt werden | Business-Logo bevorzugen, sonst Emblem/neutrales Asset; bei begrenzten Slots oder unsicherem Apple-Format als generiertes PNG (`combined_emblem`/`decorative_title`) | Implementiert inklusive Apple-PNG-Signaturpruefung |
| Emblem nach Initial-Scan | Demografie-/Gender-Emblem ersetzt Fallback | `thumbnail`/`strip` Asset, wenn oeffentliches Supabase Asset; `emblem_changed` queued Apple-Update; `combined_emblem` kombiniert Titel/Logo/Emblem | `heroImage`/`imageModulesData`; `emblem_changed` queued Google-Patch; `combined_emblem` als Bildmodul | `mainImg`/Logo-nahe Attribute, wenn Template erlaubt; `combined_emblem` bevorzugt, falls kein Hintergrundbild existiert | Teilweise | Wallets erlauben keine freie Overlay-Position; Samsung nutzt separaten Partner-Update-Pfad statt `wallet_update_queue` | Emblem als Plattform-Bildfeld oder generiertes kombiniertes Asset | Implementiert inklusive Initial-Scan-Queue-Update und `combined_emblem` PNG fuer Apple/Google/Samsung |
...
| Fall | Verhalten |
|---|---|
| Custom Font | Native Systemschrift; `decorative_title` wird serverseitig als PNG erzeugt, wenn kein Logo vorhanden ist, ein Titelbild-Flag gesetzt wird oder Apple fuer ein unsicheres Logoformat einen echten PNG-Fallback braucht |
| Kombinierte Emblemflaeche | `combined_emblem` erzeugt serverseitig eine gemeinsame Branding-/Titel-/Emblem-Grafik fuer Apple `strip/thumbnail`, Google Bildmodule/Hero-Fallback und Samsung `mainImg` |
