...
| Emblem nach Initial-Scan | Demografie-/Gender-Emblem ersetzt Fallback | `thumbnail`/`strip` Asset, wenn oeffentliches Supabase Asset; `emblem_changed` queued Apple-Update | `heroImage`/`imageModulesData`; `emblem_changed` queued Google-Patch | `mainImg`/Logo-nahe Attribute, wenn Template erlaubt | Teilweise | Wallets erlauben keine freie Overlay-Position; Samsung nutzt separaten Partner-Update-Pfad statt `wallet_update_queue` | Emblem als Plattform-Bildfeld oder generiertes kombiniertes Asset | Implementiert inklusive Initial-Scan-Queue-Update fuer Apple/Google |
